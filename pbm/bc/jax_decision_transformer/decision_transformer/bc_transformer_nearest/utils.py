...
def evaluate_on_env(policy_model,
    ...
    for _, key_i in enumerate(jax.random.split(_key, num=num_eval_ep)):
        ...
        state, delay_data_base, delay_roomba_pos, mask = organaize_state(state, env, args, primitive_num, state_mean, state_std, delay_data_base)
        ...
        for t in range(max_test_ep_len):
            ...
            state, delay_data_base, delay_roomba_pos, mask = organaize_state(state, env, args, primitive_num, state_mean, state_std, delay_data_base, delay_roomba_pos, t+1)

            total_reward += running_reward
            last_iou = loss_info['incremental_iou']
           
            # save_gif.
            if (save_gif) & (i_train_iter % giff_save_iters == 0) & ((t+1)%1000==0 or t == 0):
        ...
        if (save_gif) & (i_train_iter % giff_save_iters == 0):
            if real_demo:
                imgs[0].save(f"{log_dir}/{args.env_name}_{last_iou}_{total_eval_step+t}.gif", save_all=True, append_images=imgs[1:], loop=0)
                with open(f'{log_dir}/iou_{args.env_name}_{last_iou}_{i_train_iter}_{total_eval_step+t}.txt', 'w') as f:
...
def organaize_state(state, env, args, primitive_num, state_mean, state_std, delay_data_base, delay_roomba_pos=None, t=0):
    ## change observation
