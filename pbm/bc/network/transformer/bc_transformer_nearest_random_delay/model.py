...
from models.nearest_attention import MultiHeadDotProductAttentionWithWeight
...
class TransformerModel(linen.Module):
  ...
  def __call__(self, data: jnp.ndarray, src_mask: jnp.ndarray = None):
    ...
    # decoder
    print(output.shape, data.shape, self.condition_decoder)
    output = linen.Dense(
      self.output_size,
      kernel_init=jax.nn.initializers.uniform(scale=0.1),
...
def make_transformer(output_size: int,
                     ...
                     condition_decoder: bool = True) -> TransformerModel:
