...
from transformer.bc_transformer_nearest.utils import (
    ...
    evaluate_on_env, load_params, save_params)
...
def get_args():
    ...
    parser.add_argument("--policy_params_path", type=str, default='/root/roomba_hack/pbm/bc/network/dt_runs/bc_transformer_no_attention_0_multi_bc_rope_6/seed_0/22-11-25-13-51-19/model_5000.pt')
...
def visualize_mtrix(attn_weights_list, primitive_num, log_dir, args):
    # setting
    args.matrix_freq = 1
...
def visualize_mtrix_with_picture(attn_weights_list, primitive_num, log_dir, imgs, args):
    # setting
    layer_size = len(attn_weights_list[0])
    head_size = attn_weights_list[0][0].shape[1]
    assert args.select_layer < layer_size, f'max layer size {layer_size-1}, but you give me {args.select_layer}'
...
def visualize_mtrix_pick(attn_weights_list, primitive_num, log_dir, imgs, args):
    # setting
    layer_size = len(attn_weights_list[0])
    head_size = attn_weights_list[0][0].shape[1]
    assert args.select_layer < layer_size, f'max layer size {layer_size-1}, but you give me {args.select_layer}'
...
def main(args):
    ...
    visualize_mtrix_with_picture(attn_weights_list, primitive_num, log_dir, imgs, args)
