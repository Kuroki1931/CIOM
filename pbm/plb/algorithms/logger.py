...
class Logger:
    def __init__(self, path):
        ...
        self.prefix = 'train'
        self.keys = ['step', 'reward', 'loss', 'sdf', 'density', 'contact', 'total_iou', 'last_iou', 'iou']
    ...
    def step(self, state, action, reward, next_state, done, info):
        ...
        self.values['iou'] = info['iou']

        if done:
            fps = self.steps / (time.time() - self.start)

            print(f"STEP: {self.steps}, reward {self.values['reward']} last_iou {self.values['last_iou']} iou {self.values['iou']} loss {self.values['loss']} fps: {fps}")
