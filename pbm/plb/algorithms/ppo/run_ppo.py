...
def train_ppo(env, path, logger, old_args):
    ...
    with open(f'{args.save_dir}/ppo_args.txt', mode="w") as f:
    ...
    for j in range(num_updates):
        ...
        if (j % args.save_interval == 0
            ...
            ], os.path.join(save_path, args.env_name + f"iter{j}.pt"))


            rollouts_eval = RolloutStorage(args.num_steps, args.num_processes,
                              envs.observation_space.shape, envs.action_space,
                              actor_critic.recurrent_hidden_state_size)
            obs = envs.reset()
            env.reset()
            rollouts_eval.obs[0].copy_(obs)
            rollouts_eval.to(device)
            frames = []
            actions = []
            for step_eval in range(env._max_episode_steps):
                value, action, action_log_prob, recurrent_hidden_states = actor_critic.act(
                                rollouts_eval.obs[step_eval], rollouts_eval.recurrent_hidden_states[step_eval],
                                rollouts_eval.masks[step_eval])
                # Obser reward and next obs
                obs, reward, done, infos = envs.step(action)
                masks = torch.FloatTensor(
                            [[0.0] if done_ else [1.0] for done_ in done])
                bad_masks = torch.FloatTensor(
                    [[0.0] if 'bad_transition' in info.keys() else [1.0]
                    for info in infos])
                rollouts_eval.insert(obs, recurrent_hidden_states, action,
                                action_log_prob, value, reward, masks, bad_masks)
                ...
                frames.append(pimg)
            frames[0].save(f'{args.save_dir}/demo_iter{j}.gif', save_all=True, append_images=frames[1:], loop=0)
            actions = np.array(actions)
            np.save(f"{args.save_dir}/action_iter{j}.npy", actions)
            obs = envs.reset()
