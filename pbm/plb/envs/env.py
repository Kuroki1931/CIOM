...
class PlasticineEnv(gym.Env):
    def __init__(self, cfg_path, version, nn=False):
        ...
        self.cfg = self.load_varaints(cfg_path, version)
        self.taichi_env = TaichiEnv(self.cfg, nn) # build taichi environment
        self.cfg_save = self.cfg
        self.taichi_env.initialize()
        self.cfg = self.cfg.ENV
    ...
    def initialize_update_target(self, target_density_numpy, target_density_path):
        self.taichi_env.initialize_update_target(target_density_numpy, target_density_path)
    ...
    def get_obs(self, t=0):
        ...
        return np.concatenate((np.concatenate((x, v), axis=-1).reshape(-1), s.reshape(-1)))
    
    def get_goal_obs(self, args):
        goal_particles = np.load(f'/root/roomba_hack/pbm/tasks/multi_bc_rope/imgs/{args.task_version}/goal_state.npy')
        goal_state = goal_particles.reshape(-1)
        return goal_state
    ...
    def create_grid_mass(self):
        self.taichi_env.create_grid_mass()
    
    def get_primitive_dim(self):
        return self.taichi_env.primitives.action_dim

    def get_state(self):
        return self.taichi_env.get_state()
    
    def set_state(self, state, softness, is_copy):
        self.taichi_env.set_state(state, softness, is_copy)
    
    def compute_loss(self):
        loss = self.taichi_env.compute_loss()
        return loss
