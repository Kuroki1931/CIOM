...
class Loss:
    ...
    def initialize(self, target_density_path=None):
        ...
        if target_density_path:
            target_path = target_density_path
        else:
    ...
    @ti.kernel
    def compute_primitives_collision_loss_kernel(self, f: ti.i32):
        pass
        # for primitive_i in ti.static(self.primitives):
        #     for primitive_j in ti.static(self.primitives):
        #         xi = primitive_i.position[f][0]
        #         yi = primitive_i.position[f][2]
        #         xj = primitive_j.position[f][0]
        #         yj = primitive_j.position[f][2]
        #         if primitive_i == primitive_j:
        #             self.loss[None] += 0
        #         elif (xi-xj)**2 + (yi-yj)**2 < 0.0256:
        #             self.loss[None] += 0.25
        #         else:
        #             self.loss[None] += 0
    ...
    def compute_loss_kernel(self, f):
        ...
        if len(self.primitives) > 0:
            ...
            self.compute_primitives_collision_loss_kernel(f)
    ...
    def compute_loss_kernel_grad(self, f):
        ...
        if len(self.primitives)>0:
            ...
            self.compute_primitives_collision_loss_kernel.grad(f)
            if self.soft_contact_loss:
    ...
    def compute_loss(self, f):
        ...
        incremental_iou = max(min((loss_info['iou']-self.init_iou)/(loss_info['target_iou'] - self.init_iou), 1), 0)
        loss_info['reward'] = r
