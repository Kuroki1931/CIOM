...
class two_roomba_one_rope_one_obj_real2sim:
        ...
        while not done:
            try:
                ...
                if self.create_grid_mass:
                    self.frame = cv2.imread(f'/root/roomba_hack/pbm/tasks/two_roomba_one_rope_one_obj/demo/{task_ver}/goal.jpg')
                else:
                    print('set picture')
                ...
                # # roomba1
                roomba1_id = 10
                roomba1UL = np.array([-corners[np.where(ids == roomba1_id)[0][0]][0][0][1], corners[np.where(ids == roomba1_id)[0][0]][0][0][0]]) - self.origin
                roomba1UR = np.array([-corners[np.where(ids == roomba1_id)[0][0]][0][1][1], corners[np.where(ids == roomba1_id)[0][0]][0][1][0]]) - self.origin
                roomba1BR = np.array([-corners[np.where(ids == roomba1_id)[0][0]][0][2][1], corners[np.where(ids == roomba1_id)[0][0]][0][2][0]]) - self.origin
                roomba1C = (roomba1UL + roomba1UR)/2 +  + (roomba1UR - roomba1BR) * 2.5 /13.7
                roomba1_first_vec =  roomba1UR - roomba1BR
                # roomba2
                roomba1_id = 15
                roomba2UL = np.array([-corners[np.where(ids == roomba1_id)[0][0]][0][0][1], corners[np.where(ids == roomba1_id)[0][0]][0][0][0]]) - self.origin
                roomba2UR = np.array([-corners[np.where(ids == roomba1_id)[0][0]][0][1][1], corners[np.where(ids == roomba1_id)[0][0]][0][1][0]]) - self.origin
                roomba2BR = np.array([-corners[np.where(ids == roomba1_id)[0][0]][0][2][1], corners[np.where(ids == roomba1_id)[0][0]][0][2][0]]) - self.origin
                roomba2C = (roomba2UL + roomba2UR)/2 +  + (roomba2UR - roomba2BR) * 2.5 /13.7
                roomba2_first_vec = roomba2UR - roomba2BR
                # obj
                objUL = np.array([-corners[np.where(ids == 9)[0][0]][0][0][1], corners[np.where(ids == 9)[0][0]][0][0][0]]) - self.origin
                objUR = np.array([-corners[np.where(ids == 9)[0][0]][0][1][1], corners[np.where(ids == 9)[0][0]][0][1][0]]) - self.origin
                objBR = np.array([-corners[np.where(ids == 9)[0][0]][0][2][1], corners[np.where(ids == 9)[0][0]][0][2][0]]) - self.origin
                objBL = np.array([-corners[np.where(ids == 9)[0][0]][0][3][1], corners[np.where(ids == 9)[0][0]][0][3][0]]) - self.origin
                objC = (objUL +objUR + objBR + objBL)/4
                ...
                sim_obj_init = np.insert(objC * self.sim_unit_aruco2sim, 1, 0.02)
                self.roomba_init = [str(tuple(sim_roomba1_init)), str(tuple(sim_roomba2_init)), str(tuple(sim_obj_init))]
    ...
    def get_obj_particle(self, hight=0.06):
        hsv = cv2.cvtColor(self.frame, cv2.COLOR_BGR2HSV)
        lower = np.array([56, 35, 126])
        ...
        if self.create_grid_mass:
            output_dir = f'/root/roomba_hack/pbm/tasks/two_roomba_one_rope_one_obj/demo/{self.task_ver}'
