...
class one_roomba_one_box_real2sim:
    def __init__(self, create_grid_mass, task_ver):
        done = False
        maximum = 0
        while not done:
            try:
                ...
                if self.create_grid_mass:
                    frame = cv2.imread(f'/root/roomba_hack/pbm/tasks/one_roomba_one_box/demo/{task_ver}/goal.jpg')
                else:
                    print('set picture')
                ...
                # # box 
                boxUL = np.array([-corners[np.where(ids == 4)[0][0]][0][0][1], corners[np.where(ids == 4)[0][0]][0][0][0]]) - origin
                boxUR = np.array([-corners[np.where(ids == 4)[0][0]][0][1][1], corners[np.where(ids == 4)[0][0]][0][0][0]]) - origin
                boxBR = np.array([-corners[np.where(ids == 4)[0][0]][0][2][1], corners[np.where(ids == 4)[0][0]][0][2][0]]) - origin
                boxC = (boxUL + boxBR)/2
                ...
                self.roomba_init = [str(tuple(sim_roomba1_init))]
                self.roomba_first_vec = [roomba1_first_vec]
                # box
    ...
    def get_obj_particle(self, n_particles=2500, box_hight=0.1):
        ...
        if self.create_grid_mass:
            output_dir = f'/root/roomba_hack/pbm/tasks/one_roomba_one_box/demo/{self.task_ver}'
